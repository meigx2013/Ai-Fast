...
生成 YAML DSL 用例时，**所有用例数据必须严格来自用户输入的 test-cases 描述，严禁自由发挥**。具体规则：

1. **URL 严格照搬**：用户描述中出现的 URL（如 `https://platform-dt02.asdm.ai/`）必须原样写入用例，不得替换、省略或编造其他 URL
2. **输入值严格照搬**：用户描述中的输入值（如邮箱 `super-admin@asdm.ai`、密码 `superadmin@20260214`）必须原样写入用例，不得编造、修改或替换为其他值
3. **操作步骤严格照搬**：用户描述的每一步操作必须逐一转换为 YAML Step，不得增删用户未描述的操作步骤（补充 `wait`/`assert` 除外）
4. **预期结果严格照搬**：用户描述中的"预期结果"必须原样转换为断言，不得编造用户未描述的预期结果
5. **前置条件严格照搬**：用户描述中的"前置条件"必须原样记录到 `metadata.prerequisites`，不得编造或修改
6. **选择器允许推断**：选择器（CSS selector）是唯一允许 AI 推断的字段，因为用户通常不会提供精确选择器，但推断时须保守且标注 `confidence`
7. **逆向/边界值场景的数据约束**：当 `scenarioType=all` 时，逆向和边界值场景只能基于用户描述中**已出现的输入字段**生成变体（如空值、错误值），不得编造用户未提及的输入字段或业务规则
8. **禁止编造场景**：不得凭空创造用户描述中不存在的测试场景、页面、功能或数据
...
3. 为每个用例建立结构化摘要（**所有数据值必须逐字从用户描述中提取，禁止修改或编造**）：
...
- **逆向场景只能基于用户描述中已出现的输入字段生成变体**，不得编造用户未提及的字段或业务规则：
  - 必填字段为空（如用户描述了邮箱字段→空邮箱，用户描述了密码字段→空密码）
  - 格式错误（如用户描述了邮箱→非法邮箱格式）
  - 错误凭证（如用户描述了密码→错误密码）
  - **禁止**：用户未描述"验证码"字段时，不得编造"验证码错误"逆向场景
- 场景命名规则：`{用例名}-{逆向场景描述}`
- 示例：`asdm-login-wrong-password`、`asdm-login-empty-email`

#### 2.3 边界值场景（Boundary Value）— 仅当 scenarioType=all 时生成

- **仅基于用户描述中已出现的输入字段**推断边界值
- 边界值测试类型：
  - 邮箱：最大长度、最小长度、特殊字符（仅在用户描述中出现了邮箱字段时）
  - 密码：最大长度、最小长度、特殊字符（仅在用户描述中出现了密码字段时）
  - **禁止**：用户未描述的字段不得生成边界值场景
  - 如用户描述中无明确边界信息，则跳过边界值场景
- 场景命名规则：`{用例名}-{边界值描述}`
...
|:-:|---------|--------|:----:|--------|:------:|:------:|:------:|------|
| 1 | 001_system-login | 001_系统登录.yaml | 正向 | 系统登录 | 4 | 7 | 3 | auth, login, P1 |
...
| 禁止行为 | 正确做法 | 示例 |
|---------|---------|------|
| 编造用户未提供的 URL | 严格使用用户描述中的 URL | 用户说 `https://platform-dt02.asdm.ai/` → 必须原样写入，不得改为 `https://example.com` |
| 修改用户提供的输入值 | 逐字复制用户的输入值 | 用户说密码 `superadmin@20260214` → 不得改为 `admin123` |
| 增加用户未描述的操作步骤 | 只转换用户描述的步骤 | 用户描述5步 → 只生成5个Step，不得自行增加"检查Cookie"等步骤 |
| 编造用户未描述的预期结果 | 只将用户的"预期结果"转为断言 | 用户说"跳转到首页" → 断言页面跳转，不得额外添加"检查用户头像"断言 |
| 编造用户未提及的逆向场景 | 逆向场景只基于用户描述的输入字段 | 用户只描述了邮箱+密码 → 逆向只能是空邮箱/空密码/错误密码，不得编造"验证码错误" |
| 替换用户的用例名称 | 使用用户描述中的用例名称 | 用户说"系统登录" → name 包含 `system-login`，不得改为 `user-auth` |

**选择器是唯一允许 AI 推断的字段**，但推断时须保守且在审核提示中标记。
