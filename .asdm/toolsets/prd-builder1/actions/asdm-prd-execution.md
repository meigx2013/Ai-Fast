...
- Write necessary tests (if project has test infrastructure) - **SEE SECTION 3**
- Use available tools to create, modify, or delete files as needed
- Ensure the implementation is complete and functional

**Before starting**:
- Check if project has test files (see Section 3)
- If yes, plan to write tests
- Identify validation commands from Task PRD Section 4.1
...
- After making significant code changes, run quick validation checks:
  ...
  - For tests: Run test command if tests exist
...
- **Report validation results**:
  ...
  - Include test results if tests exist (passed/failed counts)
...
**Step 1: Check if project has test infrastructure**

Search for test files:
```bash
search_file("*.spec.js", recursive=True)  # JavaScript/TypeScript
search_file("*Test.java", recursive=True) # Java
```

Check for test configs:
- `jest.config.js`, `vitest.config.js` (JavaScript)
- `package.json` with `"test"` scripts
- `pom.xml` with junit/maven-surefire (Java)

**Step 2: Determine if tests are required**

```
IF (project has test files OR Task PRD has test commands) AND (task modifies code)
THEN tests are REQUIRED
ELSE tests are NOT required
```

**Step 3: If tests are required**

- Write unit tests for new code
- Use mocks for external dependencies (APIs, databases)
- Add tests to existing test files
- Run test command and verify all tests pass (exit code 0)
- Don't mark task as DONE until tests pass

**Example with mocked API:**
```javascript
jest.mock('@/api/login', () => ({
...
it('should validate mobile number', () => {
  expect(validateMobile('13800138000')).toBe(true);
});
```
